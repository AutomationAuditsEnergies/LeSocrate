...
class ClaudeCodeHumanizationTest(unittest.TestCase):
    def test_volume_audit_uses_pipeline_repository_for_postgres_job(self):
        rows = [{
            "folder_id": 101,
            "folder_name": "Jour 1",
            "position": 0,
        ...
        with (
            patch.object(ccms, "_course_day_budget_for_volume", return_value=budget),
            patch.object(ccms.pipeline_repo, "get_pipeline_job", return_value={"id": 13}),
            patch.object(
                ccms.pipeline_repo,
            ...
            audit = ccms.compute_volume_audit(13)
