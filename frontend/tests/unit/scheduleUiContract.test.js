...
test('reviews the definitive schedule only when preparation is requested', async () => {
  ...
  assert.match(plannerSource, /if \(!applyAllDays \|\| !normalizedDates\.length\) return/)
  assert.doesNotMatch(plannerSource, /if \(!applyAllDays \|\| !bulkTemplateId \|\| !normalizedDates\.length\) return/)
  assert.match(plannerSource, /setAssignments\(assignTemplateToAll\(normalizedDates, bulkTemplateId\)\)[\s\S]*?setCustomDays\(\{\}\)/)
  ...
  assert.doesNotMatch(dashboardSource, /id="teacher-ai-voice"/)
  assert.match(dashboardSource, /Choisissez la voix du professeur/)
  assert.match(dashboardSource, /Continuer vers les diapositives/)
  assert.match(dashboardSource, /setPostPlanningStep\('slides'\)/)
  assert.ok(
    dashboardSource.indexOf('Choisissez la voix du professeur')
      < dashboardSource.indexOf('Nom de votre centre de formation sur les diapositives'),
  )
...
test('adds independent pedagogical blocks directly to formation days', async () => {
  ...
  assert.match(source, /removeLastScheduleSequence\(blocks\)/)
  assert.match(templateSource, /removeLastScheduleSequence\(draft\.blocks\)/)
