...
test('keeps teacher tools inside one workspace without an archive action', async () => {
  ...
  assert.match(coursesSource, /avec son audio et ses diapositives, contactez le support/)
  assert.doesNotMatch(coursesSource, /avec son audio et sa visio/)
  ...
  assert.match(coursesSource, /fillInfoClosing \? 'pointer-events-none translate-x-full opacity-95'/)
  assert.match(coursesSource, /closeFillInfo\(\(\) => \{\s*setShowFillForm\(true\)/)
  assert.match(coursesSource, /prefers-reduced-motion: reduce/)
  ...
  assert.match(coursesSource, /Aucune pause ni jointure pour cette journée\./)
  assert.match(coursesSource, /visibleJointureBreaks = \(contentScriptModal\.breaks \|\| \[\]\)\.filter\(br => br\.type === 'jointure'\)/)
  assert.match(coursesSource, /Jointures entre cours/)
  assert.match(coursesSource, /Afficher la jointure entre les cours/)
  assert.match(coursesSource, /Cours \{String\(fromCourse\)\.padStart\(2, '0'\)\} vers cours/)
  assert.match(coursesSource, /Texte générique · \{Math\.round\(br\.duration_sec \|\| 0\)\} s/)
  assert.match(coursesSource, /Texte lu par le TTS/)
