...
test('keeps teacher tools inside one workspace without an archive action', async () => {
  ...
  assert.doesNotMatch(dashboardSource, /<AudiosModal/)
  ...
  assert.match(dashboardSource, /activeTool \? 'hidden' : 'hidden sm:flex'/)
  assert.match(dashboardSource, /!activeTool \? \([\s\S]*?sm:hidden[\s\S]*?Prochaines diffusions/)
